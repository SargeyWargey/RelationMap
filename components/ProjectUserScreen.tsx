...
import { buildPersonIndex } from "@/lib/userLayout";
...
export function ProjectUserScreen({ initialGraph, databaseColors, lastSyncAt }: Props) {
  ...
  const [showDbLabels, setShowDbLabels] = useState(() => {
    if (typeof window !== "undefined") {
      const v = localStorage.getItem("user_db_labels");
  ...
  const [groupByDb, setGroupByDb] = useState(() => {
    if (typeof window !== "undefined") {
      const v = localStorage.getItem("user_group_by_db");
      if (v === "true") return true;
    }
    return false;
  });

  useEffect(() => { localStorage.setItem("user_db_labels",    String(showDbLabels)); }, [showDbLabels]);
  useEffect(() => { localStorage.setItem("user_group_by_db",  String(groupByDb)); },  [groupByDb]);
                  ...
                  filteredPersonKeys.map((key) => {
                    ...
                    return (
                      <button
                        key={key}
                        type="button"
                        onClick={() => { setActivePersonKey(key); setPickerOpen(false); }}
                        style={{
                          width: "100%",
                          display: "flex",
                          alignItems: "center",
