...
const PANEL_HEIGHT      = 420;   // SVG viewBox height per panel
const SPINE_Y           = PANEL_HEIGHT / 2;  // spine sits on the vertical midpoint
const NODE_R            = 8;     // radius of node circle on spine
const NODE_STROKE       = 2.5;
const TICK_LABEL_Y_GAP  = 16;    // px below/above spine for tick labels

// Carousel geometry
const CENTER_SCALE      = 1.0;
const CENTER_OPACITY    = 1.0;
const ADJ_OPACITY       = 0.35;  // opacity of adjacent (±1) panels
const ADJ_SCALE         = 0.88;  // scale of adjacent panels

// Grow animation
const GROW_DURATION_MS  = 3000;  // spine + nodes grow-in time
const LABEL_DELAY_MS    = 900;   // labels start appearing after spine starts

// Left offset: leave room for the database panel (~220px) + some breathing room
const LEFT_INSET = 240;
...
type TimelinePanelProps = {
  ...
  screenWidth:       number;   // container width in px (determines how many world units are visible)
};
...
function TimelinePanel({
  ...
  // Spine grows from left edge to growProgress * totalWidth
  const spineEndX = PANEL_MARGIN + (totalWidth - PANEL_MARGIN * 2) * easeOutCubic(growProgress);
  ...
  return (
    <svg
      viewBox={viewBox}
      width={screenWidth}
      height={(PANEL_HEIGHT / PANEL_WIDTH) * screenWidth}
    ...
    >
      ...
      {growProgress > 0.15 && ticks.map((tick) => {
        ...
        const tickOpacity = labelProgress * 0.55;
        return (
          <g key={tick.label} opacity={tickOpacity}>
            <line
              x1={tick.xPosition}
              y1={spineY - 6}
              x2={tick.xPosition}
          ...
          <g key={`${node.nodeId}-${idx}`} opacity={nodeOpacity}>
            ...
            <g opacity={labelOpacity}>
              {(() => {
                ...
                const labelBoxH = 56;
                const dateH = 18;
                const dbH = showDbLabels ? 16 : 0;
                const totalH = labelBoxH + dateH + dbH + 6;
                const boxX = node.xPosition - labelBoxW / 2;

                // above: stack goes upward from branch end; below: downward
                const titleY = node.side === "above"
                  ? branchEndY - 6 - totalH + labelBoxH
                  : branchEndY + 6;
...
export function UserCanvas({
    ...
    function loop(now: number) {
      ...
      if (tween) {
        ...
        if (t >= 1) {
          ...
          // Trigger grow-in for newly centered panel
          const newKey = sortedKeys[targetIdxRef.current];
          if (newKey && newKey !== lastLockedKeyRef.current) {
            lastLockedKeyRef.current = newKey;
            growStateRef.current.set(newKey, { startTime: now });
          }
        }
      }
  ...
  }, [sortedKeys, onActivePersonChange]);
    ...
    function onKey(e: KeyboardEvent) {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (node && containerWidth > 0) {
          const svgScale = (containerWidth - LEFT_INSET - 80) / PANEL_WIDTH;
          const nodeScreenX = LEFT_INSET + node.xPosition * svgScale;
          const rightEdge = containerWidth * (1 - PANEL_SCROLL_MARGIN);
          if (nodeScreenX > rightEdge) {
            const overflow = nodeScreenX - rightEdge;
            const newScroll = (timelineScrollXRef.current.get(key) ?? 0) + overflow / svgScale;
            ...
            const underflow = LEFT_INSET + 40 - nodeScreenX;
            const newScroll = (timelineScrollXRef.current.get(key) ?? 0) - underflow / svgScale;
  ...
  useEffect(() => {
    ...
    function onWheelHoriz(e: WheelEvent) {
      ...
      const svgScale = (containerWidth - LEFT_INSET - 80) / PANEL_WIDTH;
      const scrollDelta = e.deltaX / svgScale;
  ...
  // Viewport height of one panel slot = 60% of container height
  // We read it from the container ref at render time
  const containerHeight = containerRef.current?.clientHeight ?? 600;
  const panelSlotH      = containerHeight * 0.60;  // 60vh per panel
  const nowMs           = performance.now();

  // Which panels to render: active ±2
  const visibleIdxs: number[] = [];
  for (let i = Math.max(0, activeIdx - 2); i <= Math.min(N - 1, activeIdx + 2); i++) {
    visibleIdxs.push(i);
  }
  void visibleIdxs; // used for reference only
  ...
  return (
    ...
    >
      ...
      {/* Carousel strip — translate so panel[activeIdx] sits at vertical center */}
      <div style={{
        position:      "absolute",
        left:          0,
        right:         0,
        // Stack panels starting at top=0; shift by -(offsetY * panelSlotH) + half container
        top:           `calc(50% - ${panelSlotH / 2}px - ${offsetYRef.current * panelSlotH}px)`,
        display:       "flex",
        flexDirection: "column",
        willChange:    "top",
      }}>
        {sortedKeys.map((key, i) => {
          const entry    = personIndex.get(key)!;
          const delta    = i - offsetYRef.current;
          const absDelta = Math.abs(delta);

          // Only render nearby panels
          if (absDelta > 2.5) return <div key={key} style={{ height: panelSlotH }} />;

          // Scale + opacity based on distance from center
          let opacity: number;
          let scale: number;
          if (absDelta < 0.5) {
            const t = 1 - absDelta * 2;
            opacity = ADJ_OPACITY + (CENTER_OPACITY - ADJ_OPACITY) * t;
            scale   = ADJ_SCALE   + (CENTER_SCALE   - ADJ_SCALE)   * t;
          } else if (absDelta < 1.5) {
            opacity = ADJ_OPACITY;
            scale   = ADJ_SCALE;
          } else {
            const t2 = Math.max(0, 2.5 - absDelta);
            opacity  = ADJ_OPACITY * t2;
            scale    = ADJ_SCALE;
          }
          ...
          return (
            ...
            >
              {/* Name bar — always full opacity for active, distance-faded for others */}
              <PersonNameBar
                entry={entry}
                darkMode={darkMode}
                opacity={isActive ? 1 : Math.max(0.05, 0.45 - absDelta * 0.18)}
              />
              {/* Timeline SVG only for active panel */}
              {isActive && (
                <TimelinePanel
                  entry={entry}
                  groupByDb={groupByDb}
                  ...
                  screenWidth={containerWidth > LEFT_INSET + 80 ? containerWidth - LEFT_INSET - 80 : 600}
                />
              )}
            </div>
          );
